...
📂 Dataset Information:The dataset contains diagnostic measurements of breast cancer tumors obtained from digitized images of fine needle aspirate (FNA) of breast mass tissue.
Target Variable
Value	Meaning
M  -	Malignant
B  -	Benign
...
Principal Component Analysis(PCA): PCA reduces feature dimensions while preserving maximum variance.
PCA(n_components=1)
...
🤖 Machine Learning Model
Random Forest Classifier:Random Forest is an ensemble learning algorithm that builds multiple decision trees and combines them for improved prediction accuracy.
...
K-Means Clustering:Used to identify natural groupings in tumor data.
KMeans(n_clusters=5)

Euclidean Distance:Distance metric used for clustering.
Distance = √Σ(x1 − x2)²
...
Score Range: -1 to 1 [Higher values indicate better cluster separation]
silhouette_score()
